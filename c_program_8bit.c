...
void Initial_Nbitsbox(int sequence[N-1],int counter){
    ...
    for (int j=0;j<(1<<N);j++)
        for (int counter_i=0;counter_i<N-1;counter_i++)
            if ((counter>>counter_i)&1)
                truthtable_Nbit[N-1][j] ^= ((j>>counter_i)&1);
        // truthtable_Nbit[N-1][j] = 0;
    // set quadratic function (sum_{i not in seq}x_i*x_N)
    for (int i=0;i<(1<<(N-1));i++) 
        for (int j=0;j<N-1;j++) 
            if (list[j] == N-1)
                truthtable_Nbit[N-1][(1<<(N-1))+i] ^= ((i>>j)&1);
}
...
int main(){
    ...
    // for (int perm_index=0;perm_index<5040*8;perm_index++){
        ...
        for (int counter=0;counter<1<<(N-1);counter++){
            ...
            for (int arg=0;arg<(1<<(N-1));arg++)
            {
                ...
                for (int j=0;j<(1<<(N-1));j++)
                {
                    ...
                    {
                        if ((arg>>loop)&1)
                            truthtable_Nbit[N-1][(1<<(N-1))+j] = truthtable_Nbit[N-1][(1<<(N-1))+j]^((j>>loop)&1);
                    }
                }
