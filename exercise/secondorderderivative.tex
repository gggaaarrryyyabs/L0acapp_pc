...
%	Let $ V_i = \left\{x\in\F_{2^k}\middle|\TRACE\left(\theta_ix+\gamma_i\right)=0\right\} $ where $ i=1,2,3 $.
% 	Then we have  $ \\left\lvertV_i=2^{k-1} , i=1,2,3 $ and $ \#(V_i\cap V_j)=2^{k-2} $ for $ 1\le i<j\le 3 $ by Lemma \ref{lemma:N_ij}.
% 	Notice that  $ \#(V_1\cup V_2)=\#V_1+\#V_2-\#(V_1\cap V_2)=2^{k-1}-2^{k-2} $ and $ V_3\nsubseteq V_1\cup V_2 $, 
% 	otherwise 
% 	
%%	$ \sum_{i_2,i_3\in\F_2}N_{i_1,i_2,i_3}=2^{k-1} $ for $ i_1=0,1 $,  
%%    and $ \#(S_1\cup S_2\cup S_3)> 2^{k-1}+2^{k-2} $, 
%    then we have 
%    \begin{align*}
%        N_{0,0,0}&=\#(V_1\cup V_2\cup V_3)-\sum_{i=1,2,3}\#V_i+\sum_{1\le i\ne j\le 3}\#(V_i\cap V_j)\\
%        &>2^{k-1}+2^{k-2}-3\cdot 2^{k-1}+3\cdot 2^{k-2}\\
%        &>0.
%    \end{align*} 
%
%consider three equations below: 
%\begin{equation}\label{eq:3linearfunction0}\left\{\begin{alignedat}{3}
%		&\TRACE\left(\theta_1x+\gamma_1\right)=0\\ 
%		&\TRACE\left(\theta_2x+\gamma_2\right)=0\\
%		&\TRACE\left(\theta_3x+\gamma_3\right)=0.
%	\end{alignedat}\right.\end{equation}
%where $ c_1,c_2,c_3\in\F_{2^k} $ and $ \theta_1,\theta_2,\theta_3\in\F_{2^k}^* $ are distinct and satisfy $ \theta_3\ne\theta_1+\theta_2 $.
...
%    \begin{theorem}\label{thm:Num_root_inv_2deriv}
%     Let $ n \ge 3 $ be a positive integer and $ F $ be the multiplicative inverse function over $\F_{2^n}$. 
%     For any $ \gamma,\eta,\omega\in\F_{2^n} $, define
%     \[\mathcal{N}(\gamma, \eta, \omega)=\left\lvert\{x\in\F_{2^n}\middle|F(x)+F(x+\gamma)+F(x+\eta)+F(x+\gamma+\eta)=\omega\}\right\rvert.\] 
%     When $ (\gamma, \eta, \omega)$ ranges over $\F_{2^n}^3$ 
%     such that $ \gamma,\eta\in\F_{2^n}^* $ and $ \gamma\ne\eta $, we have
%     \[\mathcal{N}(\gamma, \eta, \omega)=\left\{\begin{aligned}
%         0, & 3 \cdot 2^{3 n-2}-2^{2 n+1}-\left(10(-1)^n-3(-1)^n \widehat{I_1}(1)+1\right) \cdot 2^{n-2} \\
%         &-\frac{3}{4}(-1)^n \widehat{I}_1(1)+\frac{5}{2}(-1)^n+\frac{3}{2}[\text {times}] \\
%         4, & 2^{3 n-2}-5 \cdot 2^{2 n-2}+\left(10(-1)^n-3(-1)^n \widehat{I_1}(1)+8\right) \cdot 2^{n-1} \\
%         &+\frac{3}{2}(-1)^n \widehat{I}_1(1)-5(-1)^n-3[\text {times}] \\
%         8, & 2^{2 n-2}+\left(3(-1)^n \widehat{I}_1(1)-10(-1)^n-7\right) \cdot 2^{n-2} \\
%         &-\frac{3}{4}(-1)^n \widehat{I}_1(1)+\frac{5}{2}(-1)^n+\frac{3}{2}[\text {times}]. 
%         \end{aligned}\right.\]
%    \end{theorem}
...
\section{Profile of Dillon bent functions}
    ...
    \begin{enumerate}[label=\arabic{*})]
        ...
        \end{enumerate}
    \end{enumerate} 
    % Note that in case 2), we always find 
    % $ \mu=\frac{\lambda(\alpha_2^2+\beta_2^2+\alpha_2\beta_2)}{\alpha_2^2\beta_2+\alpha_2\beta_2^2} $, 
    % making  $ \{0,\alpha_2,\beta_2,\alpha_2+\beta_2\} $ are four solutions of \eqref{eq:coefficient}. 
    % Therefore, it's clear that there exists $ (\mu,\nu) $ such that equation \eqref{eq:coefficient} 
    % has solutions for all points $ \alpha=(\alpha_1,\alpha_2),\beta=(\beta_1,\beta_2)\in\F_{2^k}\times\F_{2^k}^* $, $ \alpha_2\ne\beta_2 $ with $ \lambda\in\F_{2^k}^* $.


    % Thus, we conclude that \eqref{eq:coefficient} has $ 0 $ solution, $ 4 $ solutions, 
    % (which are $ \{0,\alpha_2,\beta_2,\alpha_2+\beta_2\} $ or $\{y_0,y_0+\alpha_2,y_0+\beta_2,y_0+\alpha_2+\beta_2\} $), 
    % $ 8 $ solutions or $ 2^k $ solutions. 
    % (which are $ \{0,\alpha_2,\beta_2,\alpha_2+\beta_2,y_0,y_0+\alpha_2,y_0+\beta_2,y_0+\alpha_2+\beta_2\} $).
...
\end{equation}
    ...
    \begin{remark}
        ...
        % of equaiton \eqref{eq:coefficient},  
    ...
    % For every points $ \alpha=(\alpha_1,\alpha_2) $ and $ \beta=(\beta_1,\beta_2) $, equaiton 
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            &\ge 
            % 2^{n-1}-\frac{1}{2}\sqrt{\sum_{\alpha_1\in\F_{2^k}}\sqrt{2^{2n}-2\sum_{\beta\in\F_{2^n}} nl(D_{\beta}D_{(\alpha_1,0)}f)}+\sum_{\alpha=(\alpha_1,\alpha_2)\in\F_{2^k}\times\F_{2^k}^*}\sqrt{2^{2n}-2\sum_{\beta=(\beta_1,\beta_2)\in\F_{2^k}^2} nl(D_{\beta}D_{\alpha}f)}}
        \end{align*}
